...
`pip-protected-fixture-discovery.yml` reads `ODDS_API_KEY`, `SOCCERDATA_API_KEY`, `SPORTS_GAME_ODDS_KEY`, and `PIP_VALIDATION_FIXTURE_CODE` only inside this protected environment. It selects the earliest upcoming Norwegian Eliteserien event with available head-to-head odds, requires an unambiguous same-home/same-away match from Soccerdata API, and adds SportsGameOdds only when its team-name match is also unambiguous. It normalizes provider metadata and creates a one-day registration-SQL artifact. Raw responses, credentials, fixture codes, provider event IDs, team names, and SQL content are not logged. A missing or ambiguous required second-provider match fails closed and creates no artifact; an unavailable optional provider is omitted rather than fabricated.
